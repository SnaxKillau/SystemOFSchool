...
function ReportDetail() {
    ...
    const data = useSelector(state => state.detailofreport)
    const dispatch = useDispatch()
    const ParamId = useParams()

   useEffect(() => {
    dispatch(reportDetail(ParamId.id))
    console.log(data)
  ...
  return (
    ...
    <Menu></Menu>
    {
       ...
       <h1 className='ml-5 mt-10 sm:ml-20 font-semibold text-xl'>{data.report.data?.attributes.information}</h1>
